...
export interface PixelBlastProps {
  /** Shape variant of the pixel pattern */
  variant?: PixelBlastVariant
  /** Size of individual pixels in device-independent px */
  pixelSize?: number
  /** Base color — defaults to theme primary (#FF7A18) */
  color?: string
  ...
  patternScale?: number
  /** Density of filled pixels (0–1) */
  patternDensity?: number
  /** Enable cursor-following liquid distortion */
  liquid?: boolean
  /** Strength of the liquid distortion */
  liquidStrength?: number
  /** Cursor influence radius for liquid */
  liquidRadius?: number
  /** Random jitter applied to pixel coverage (0–1) */
  pixelSizeJitter?: number
  /** Show expanding ripple rings on click */
  enableRipples?: boolean
  /** Brightness multiplier for ripples */
  rippleIntensityScale?: number
  /** Width of the ripple ring */
  rippleThickness?: number
  /** Expansion speed of ripple rings */
  rippleSpeed?: number
  /** Oscillation frequency of liquid wobble */
  liquidWobbleSpeed?: number
  /** Pause the animation when off-screen */
  autoPauseOffscreen?: boolean
  /** Animation time-scale multiplier */
  speed?: number
  /** Transparent background (true) or solid black (false) */
  transparent?: boolean
  /** Fraction of screen edge over which pixels fade to 0 */
  edgeFade?: number
  /** Film-grain noise amount (0 = off) */
  noiseAmount?: number
}

// ── Touch-trail texture (64 × 64 canvas) ──────────────────────────────────────
...
/**
 * INSPRANO theme primary color — used when no explicit `color` prop is given.
 */
const THEME_COLOR = "#FF7A18"

// ── Component ──────────────────────────────────────────────────────────────────

/**
 * `PixelBlast` renders a full-bleed, GPU-driven dithered pixel pattern with
 * optional click ripples and cursor-following liquid distortion.
 *
 * Place it as a background layer and content on top via `z-index` / absolute
 * positioning.
 *
 * @example
 * ```tsx
...
export function PixelBlast({
  ...
  const visibilityRef = React.useRef({ visible: true })
  ...
  const prevConfigRef = React.useRef<ReinitConfig | null>(null)
