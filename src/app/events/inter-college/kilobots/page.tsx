...
import { interCollegeEvents, kilobotsEvents } from "@/data/eventData";
import { ChevronLeft } from "lucide-react";

const category = interCollegeEvents.find((e) => e.id === "inter-kilobots")!;

export default function KiloBotsPage() {
  return (
            ...
            <div className="flex flex-col justify-center space-y-6 sm:space-y-8">
              ...
              <GlowContainer className="border-l-2 border-primary/50 bg-primary/5 pl-4">
                <p className="font-mono text-xs italic text-primary">
                  Build the bot. Enter the arena. Destroy the competition.
                </p>
              </GlowContainer>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <Stat label="EVENTS" value={kilobotsEvents.length.toString()} unit="TOTAL" />
                <Stat label="FORMAT" value="Team" unit="BASED" />
                <Stat label="DOMAIN" value="Robo" unit="COMBAT" />
        ...
        <div className="container relative z-10 mx-auto px-4">
          <div className="mb-6 space-y-2 border-l-4 border-primary/50 bg-panel px-4 py-3 sm:mb-8 sm:px-6 sm:py-4">
            ...
            <p className="font-mono text-xs text-foreground/70 sm:text-sm">
              {kilobotsEvents.length} events under KiloBots
            </p>
          </div>

          <div className="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
            {kilobotsEvents.map((event) => (
              <Card
                key={event.id}
